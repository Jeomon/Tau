...
class SchedulerState:
    ...
    def update_prompt(self, task_id: str, prompt: str) -> bool:
        task = self.tasks.get(task_id)
        if not task:
            return False
        task.prompt = prompt
    ...
    def update_interval(self, task_id: str, interval_s: float) -> bool:
        task = self.tasks.get(task_id)
        if not task:
            return False
        task.interval_s = interval_s
        task.jitter_s = self._jitter(task_id, interval_s)
        task.next_run_at = time.time() + interval_s + task.jitter_s
        task.pending = False
        self._persist()
        return True
