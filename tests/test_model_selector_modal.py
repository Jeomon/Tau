...
class TestRender:
    ...
    def test_text_model_shows_context_and_modalities(self):
        modal = ModelSelectorModal(_sections())
        lines = render_to_lines(modal, 100)
        out = "\n".join(lines)
    ...
    def test_image_and_video_models_show_output_modality(self):
        ...
        image_out = "\n".join(render_to_lines(ModelSelectorModal(sections, initial="image"), 100))
        video_out = "\n".join(render_to_lines(ModelSelectorModal(sections, initial="video"), 100))
