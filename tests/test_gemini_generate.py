...
# AskUserParams lives in the bundled ask_user extension. Loaded straight from
# the file rather than imported: the package's __init__ is the extension entry
# point, and importing it would run register() as a side effect of a test that
# only wants the schema.
_ASK_USER_SCHEMA = (
    Path(__file__).parent.parent / "tau" / "builtins" / "extensions" / "ask_user" / "schema.py"
)
