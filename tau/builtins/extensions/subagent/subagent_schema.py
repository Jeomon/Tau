...
class SubagentParams(BaseModel):
    ...
    agent_scope: Literal["user", "project", "both"] = Field(
        default="user",
        description=(
            "Which agent directories to search. 'user' (default) only loads "
            "~/.tau/agents. 'project' or 'both' also load .tau/agents from the "
            "current project — only use this for trusted repositories."
        ),
    )
