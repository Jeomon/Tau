...
Spawns a separate `tau` process (in `--mode json --ephemeral` non-interactive
mode) for each subagent invocation, giving it an isolated context window.
NDJSON hook events on stdout (`message_end`, `tool_execution_start`, ...) are
parsed back into usage stats and a lightweight activity log.

Supports two modes (a single task is just a one-item list in either):
...
async def run_single_agent(
    *,
    ...
    args = ["--mode", "json", "--ephemeral", "--quiet"]
    ...
    def __init__(self, runtime_ref: Any) -> None:
        ...
        super().__init__(
            ...
            description=(
                ...
                "'spawn' (list of {agent, task}) to run tasks concurrently (max 8, 4 at "
                "a time), or 'chain' (list of {agent, task}) to run steps one after "
                "another, where each step's task may contain '{previous}' for the prior "
                "step's output. A single task is just a one-item list in either field. "
                "Ships with 'scout' (fast read-only recon), 'planner', 'reviewer', and "
                f"'worker' (full access) out of the box. Add your own in {user_agents_dir}. "
                "To enable project-local agents in .tau/agents, set agent_scope='both' or "
