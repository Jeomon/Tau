...
class MessageView:
    ...
    def __init__(self, text: str, *, role: MessageRole, timestamp: float | None = None) -> None:
        self._text = text
        self._role = role
    ...
    def render(self) -> RenderedMessage:
        ...
        root = ui.column().classes(f"w-full gap-0 tau-msg-row {self._alignment_class()}")
        with root:
            ...
            with ui.row().classes("items-center gap-1 px-1 tau-msg-meta"):
                if self._role == "assistant":
                    ui.button(
                        icon="content_copy", on_click=lambda: ui.clipboard.write(content.content)
