...
async def run_serve_mode(runtime: Runtime, host: str = "127.0.0.1", port: int = 8080) -> None:
    """Serve Tau over a browser-based UI using NiceGUI.
    ...
    def index() -> None:
        ...
        unsubs = [runtime.hooks.register(name, on_event) for name in _HOOK_NAMES]
    ...
    ui.run_with(fastapi_app, title="Tau", storage_secret="tau-serve")

    config = uvicorn.Config(fastapi_app, host=host, port=port, loop="asyncio", log_level="warning")
    server = uvicorn.Server(config)
    click.echo(f"Tau serving at http://{host}:{port}")
    await server.serve()
