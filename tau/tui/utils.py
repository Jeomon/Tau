...
def format_number(num: int | float) -> str:
    ...
    value = float(num)
    i = 0
    while value >= 1_000 and i < len(_COUNT_SUFFIXES) - 1:
        value /= 1_000
        i += 1
    ...
    if round(value, 1) >= 1_000 and i < len(_COUNT_SUFFIXES) - 1:
        value /= 1_000
        i += 1

    if round(value, 1).is_integer():
        return f"{int(round(value))}{_COUNT_SUFFIXES[i]}"
    return f"{value:.1f}{_COUNT_SUFFIXES[i]}"
