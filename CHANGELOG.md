...
-   Several clients can watch and drive one session at once, over a unix socket. `tau --mode remote` serves the session it was started with, and `tau.remote` ships the matching `RemoteClient`; the default socket is named for the session under the config directory, or `--socket PATH` picks one. Scope is deliberately one server per runtime — a `Runtime` owns its session, and hosting several would need a session-factory layer Tau does not have, so this is multi-client access to one session rather than a daemon that lists and creates them. The command and event vocabulary is not redefined: `tau/modes/rpc/mode.py` stays the single dispatcher and is handed a per-connection sink, and messages are encoded with the same `wire.json_default` stdio RPC uses, so the two transports cannot drift in what they emit. Framing is length-prefixed rather than newline-delimited, because a socket offers none of the guarantees a pipe between related processes does: reads arrive split or coalesced, and reading until a newline that never comes allocates at the peer's discretion — putting the length first is what lets an oversized frame be refused before its payload is ever buffered. Three rules govern the rest. Responses are point-to-point and events are broadcast, since a response belongs to whoever asked while an event describes the shared session. A client that stops reading is dropped rather than waited for, its outbound queue bounded, because an unbounded queue or a blocking write inside event delivery would let any observer stall the agent everyone else is using. And framing errors are fatal while protocol errors cost one reply, since a misaligned byte stream cannot be resynchronised but a single malformed message leaves the stream in step. Extension dialogs reach the attached clients rather than a stdout nothing is reading, so `ctx.select()` works over a socket instead of blocking on an answer that could never arrive; the request is broadcast and the first matching response wins. Unlike `--mode rpc`, stdout is not a protocol stream — it prints where it is listening and gets out of the way. See `docs/remote.md`

-   A remote client that lost its connection can ask for what it missed. Every settled event carries a monotonic `revision`, `ready` reports where the stream has reached, and `resume` replays from a bounded buffer. `replayed` is the entire contract: true means every settled event after the requested revision follows, false means the request cannot be satisfied — evicted, or from a different server — and the session must be refetched with `get_messages`/`get_state`. There is deliberately no partial replay, because one that reported success would leave a client confidently out of date, which is the failure replay exists to remove. That is also why revisions are assigned only to events that enter the buffer: streaming `message_update` deltas are neither retained nor numbered, since each is superseded by the `message_end` that follows, and numbering them would let a client be told it was caught up while the events in between had never been kept. The buffer is bounded by count *and* by bytes, because capping only the count lets a few large events hold megabytes while capping only the bytes lets a flood of small ones grow without limit

### Changed

-   pyright resolves the project environment instead of whichever interpreter it finds first. Run through `uv run` it always did, since uv exports `VIRTUAL_ENV`; invoked directly as `.venv/bin/pyright` it fell back to the system Python — on one machine that meant 3.14 plus `.pth` files from unrelated projects, and five errors that do not exist under the environment Tau actually runs on. A gate whose answer depends on how it is invoked is worse than a slow one, because the difference looks like a defect in the code. `venvPath`/`venv` pin the environment and `pythonVersion` pins the language level to the floor in `requires-python` rather than the newest interpreter a developer happens to have installed — local runs were assuming 3.13 while CI assumes 3.12, so syntax a supported user cannot run would have passed the gate

-   Every GitHub Actions reference is pinned to a commit SHA, with its version in a trailing comment. A version tag is mutable: whoever controls the action repository can move `v4` and every workflow referencing it runs the new code on the next build, with no diff to review. That matters most in `cd.yml`, which holds a PyPI publishing identity through OIDC. Pinning alone would trade a supply-chain risk for a staleness one, since a pinned action never picks up its own security fixes, so `.github/dependabot.yml` bumps them monthly in a single grouped PR — one PR is far likelier to be reviewed than five that are ignored

-   CI runs the tests on macOS as well as Linux, and on Python 3.13 as well as 3.12. One combination was covered before, which left 3.13 untested despite being what contributors run, and left every platform-specific branch unexercised — including the unix socket path limit in `tau/remote`, which is 104 bytes on macOS against 108 elsewhere, so only a macOS leg covers the shorter one. Format, lint and type checks moved to their own job rather than repeating on all four legs for the same answer, and `fail-fast` is off because a matrix that stops at the first red leg cannot distinguish a version-specific failure from a platform-specific one. ruff now covers `tests/` too; the type checkers deliberately do not, and `docs/development.md` records the measurement behind that

### Fixed

-   `--prompt` and `--file` are refused in remote mode rather than accepted and discarded. Remote mode serves a session and waits, so it has no initial-message path; a scripted `tau -p "task" --mode remote` built a runtime, bound a socket, never ran the prompt, and then served forever — the invocation hung indefinitely having done nothing, with the missing work as the only symptom. The check runs before the runtime is built, so a rejected command neither pays for startup it would discard nor binds a socket it would abandon

-   Sockets left by a server that died without unwinding are swept at startup. Paths are named for their session and therefore never reused, so the replace-on-bind check never revisits one and a process killed by SIGKILL, a crash or a power loss left a file that nothing would ever remove, accumulating in the config directory indefinitely. Only sockets nothing is listening on are removed, never a regular file and never the path about to be bound — the last belongs to `RemoteServer.start`, which reports a live holder as an error rather than quietly replacing it. Remote mode also no longer leaves a SIGINT handler on the shared event loop, which would have a later run inherit a callback pointing at a dead runtime

-   `bashlex` is declared in the dev dependency group, so the shell-parsing permission tests run in CI instead of being skipped. They assert real security behaviour — that a command substitution or a chained command cannot smuggle past an allow-list — and had been silently absent from every CI run
...
-   RPC clients can read and settle the project-trust decision. Trust decides whether Tau loads a project's own `.tau/` settings, extensions and context files, and an in-process extension could already read it — but the protocol carried nothing, so a client driving a headless worker could neither see the outcome nor decide it, and an undecided project was a dead end: there is no trust prompt over RPC to answer. `ready` and `get_state` now carry `projectTrusted` and `projectTrustSource`, and a `trust` command reports the decision, sets it for the session, persists it, or forgets a stored answer. Granting mid-session reloads extensions for the reason the interactive `/trust` does: project settings are skipped at startup and context files are read while the session is built. The source matters as much as the boolean, because the resolution collapses undecided to `false` — safe as a default, ambiguous as a report — so `false` alone covers both "explicitly refused" and "never asked", and those call for opposite behaviour in an unattended run. The resolution already knew which it was and discarded it; it is now recorded as it resolves (`no-inputs`, `flag`, `policy`, `stored`, `undecided`, `session`, `default`) rather than recomputed at the RPC layer, where the precedence would have to be duplicated and could drift. `trusted` is refused unless it is an actual boolean, since `"yes"` is truthy in Python and would otherwise grant trust silently
-   The RPC `ready` line announces `protocolVersion`, `runtimeVersion` and a `capabilities` object, reversing the previous "no version or capability negotiation" stance. That stance put the cost on clients: with nothing to detect, a client had to pin a Tau version out of band and read private names to find out what the runtime could do — the capability people actually need, whether an extension's `tool_call` block is honoured, was knowable only by inspecting `_INTERCEPTABLE_EVENTS`. This announces rather than negotiates: no round trip, and no state machine added to a protocol whose whole shape is one line in, one line out. An older client ignores the new keys; a newer one treats a missing `protocolVersion` as pre-negotiation. Every capability is derived from the thing it describes — `toolCallBlocking` and `interceptableEvents` from the same set the extension bridge consults, `projectTrust` probed off the runtime that would serve it — because a capability object that drifts from its subject is worse than none, being trusted exactly where the client can no longer check. `protocolVersion` bumps only for a change a client written against the previous version cannot survive; adding a field, command or event does not
-   `tool_call` fires before a tool runs, and a handler can stop it. It is the only hook that can, and it covers every tool the model can reach — built-in, extension-registered, or arriving from anywhere else — because it sits in the engine's single execution path rather than wrapping individual tools. A handler returns `ToolCallEventResult` to `block` the call (with a `reason` the model sees in place of the result, and optional structured `metadata` for the caller), or to hand back rewritten `params`; a block wins over a rewrite, since a handler that says "not this call" cannot also mean "this call, adjusted". That makes a permission gate, an audit trail or a policy filter something an extension can implement once, rather than by shadowing each tool it wants to guard. The contract is documented and stable: see the `tool_call` row in `docs/extensions.md`
-   Extension handlers that fail while guarding a tool are now recorded and surfaced rather than read as consent. The bridge between `tau.on(...)` and the hooks bus routes interceptable events through real hook handlers so their return values are collected, and everything else through a catch-all subscriber that discards them — `tool_call` was in the second group while `tool_result` was in the first, so a gate could prompt, record a denial, and the call ran anyway, silently. That path is now covered by tests that register the way an extension actually does, through the loader, rather than attaching straight to the bus, which is how the original gap survived a passing suite. Three failure modes with the same shape went with it: a context-construction failure, an error recorded from a raising runtime lookup, and the depth-counter bookkeeping around a handler all escaped into `Hooks.emit`, which logs and returns no result — indistinguishable, on this event, from approval. Each now degrades to a recorded error with the handler still able to object. Interactive mode never wired the error callback that RPC did, so those records went nowhere and the warning went only to the log file; a crashing gate now posts a deduplicated notice to the message list
